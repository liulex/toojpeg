...
namespace TooJpeg
{
  ...
  // downsample   - if true then YCbCr 4:2:0 format is used (smaller size, obvious quality loss) instead of 4:4:4, not relevant for grayscale
  // comment      - optional JPEG comment (0/NULL if no comment), must not contain ASCII code 0xFF
    bool saveJpeg(const QString& fileName, const QImage& image,
                  unsigned char quality = 90, bool downsample = false, const char* comment = nullptr);
} // namespace TooJpeg
